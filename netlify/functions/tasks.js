  ...
  try {
    ...
    const queryFilter = {
      or: [
        ...
        { and: [
            ...
            { property: "Fecha límite", date: { on_or_after: thirtyDaysAgo } },
          ] },
      ],
    };

    while (hasMore) {
      const body = { sorts: [{ property: "Fecha límite", direction: "ascending" }], page_size: 100, filter: queryFilter };
    ...
    const tasks = allResults.map((page) => {
      ...
      return {
        ...
        creadoPor: p["Creado por"]?.select?.name || "",
        editadoPor: p["Última edición por"]?.select?.name || "",
      };
    });
