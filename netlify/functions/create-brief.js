exports.handler = async function (event) {
  ...
  try {
    ...
    const properties = {
      ...
      Estado: { status: { name: entry.estado || "Sin empezar" } },
      Visibilidad: { select: { name: entry.visibilidad || "Activa" } },
      "Creado por": entry.autor ? { select: { name: entry.autor } } : { select: null },
      "Última edición por": entry.autor ? { select: { name: entry.autor } } : { select: null },
